...
library(devtools)
Sys.setenv(GITHUB_PAT = "6d79dc3951ecb3fac8b329dd8019195686c02179")
install_github("DansenCode/AnnoCrawler")
library(AnnoTools)
...
# temp
```{r, fig.width=42, fig.height=4}

path <- "/Users/ryankellogg/Downloads/Ryan-Data/hr/"
files <- dir(path, pattern = "*.json")
df_hr = data.frame()
f=files[1]
for (f in files) {
  a=stream_in(file(paste0(path,f)))
  df = data.frame(a[[3]][1],Date=a$Data_Date)
  df = unite(df,DT,Date,Time,sep=" ")
  df_hr = rbind(df_hr,df)
}

df_hr$DT2 = as.POSIXct(df_c$DT, tz = "America/Los_Angeles", format="%Y-%m-%d %H:%M:%S")
df_hr$Type="HR"

path <- "/Users/ryankellogg/Downloads/Ryan-Data/st/"
files <- dir(path, pattern = "*.json")
df_st = data.frame()
f=files[1]
for (f in files) {
  ...
  df = data.frame(a[[3]][1],Date=a$Data_Date)
  df = unite(df,DT,Date,Time,sep=" ")
  df_st = rbind(df_st,df)
}
df_st$DT2 = as.POSIXct(df_st$DT, tz = "America/Los_Angeles", format="%Y-%m-%d %H:%M:%S")
df_st$Type="Steps"

fitb = rbind(df_hr,df_st)
fitb$Device="Fitbit"

act = read_csv("/Users/ryankellogg/Downloads/Ryan-Data/output_actxa.csv")
act$DT2 = as.POSIXct(act$DT, format="%d/%m/%Y, %H:%M:%S", tz="Asia/Singapore")
act = gather(act,Type,Value,1:2)
act$Device="Actxa"

all = rbind(fitb,act)
all = subset(all,DT2 >= "2020-05-25" & DT2 <= "2020-06-04")
all$Value = as.numeric(all$Value)
all$Device = factor(all$Device,levels=c("Fitbit","Actxa"))
ggplot(subset(all,Type=="HR" & DT2>"2020-05-26 01:00:00" & DT2<"2020-05-26 18:00:00"),aes(DT2,Value,color=Device,group=Device))+ geom_line() + geom_point() + xlab("Time") + ylab("HR")
ggplot(subset(all,Type=="HR" & DT2>"2020-05-29 10:45:00" & DT2<"2020-05-30 11:15:00"),aes(DT2,Value,color=Device,group=Device))+ geom_line() + geom_point() + xlab("Time") + ylab("HR")

ggplot(subset(all,Type=="Steps" & DT2>"2020-05-26 01:00:00" & DT2<"2020-05-26 18:00:00"),aes(DT2,Value,color=Device,group=Device))+ geom_line()
ggplot(subset(all,Type=="Steps" & DT2>"2020-05-29 10:45:00" & DT2<"2020-05-30 11:15:00"),aes(DT2,Value,color=Device,group=Device))+ geom_line()
...
setwd("/Users/ryankellogg/Dropbox/24-7 omics/")
steps = read.csv("05_2019/iPOP1_steps_05_2019", sep="\t", header=TRUE)
hr = read.csv("05_2019/iPOP1_HR_05_2019", sep="\t", header=TRUE)
glu = read.csv("CGM_data.txt", sep="\t", header=TRUE) 
samples = read.csv("sample_registration.csv", header=T, colClasses = c("integer","factor","character","character","POSIXct","character","character","character"))

# sleep processing
a=stream_in(file("/Users/ryankellogg/Dropbox/24-7 omics/05_2019/Sleep.json"))
